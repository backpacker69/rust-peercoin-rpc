...
impl<C: RpcApi> Queryable<C> for peercoin::block::Block {
    type Id = peercoin::BlockHash;

    fn query(rpc: &C, id: &Self::Id) -> Result<Self> {
        let rpc_name = "getblock";
        let hex: String = rpc.call(rpc_name, &[serde_json::to_value(id)?, 0.into()])?;
        let bytes: Vec<u8> = peercoin::hashes::hex::FromHex::from_hex(&hex)?;
        Ok(peercoin::consensus::encode::deserialize(&bytes)?)
    }
}

impl<C: RpcApi> Queryable<C> for peercoin::transaction::Transaction {
    type Id = peercoin::Txid;
