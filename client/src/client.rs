...
use crate::{peercoin, deserialize_hex};
use bitcoin_private::hex::exts::DisplayHex;
use jsonrpc;
use serde;
use serde_json;

use crate::peercoin::address::{NetworkUnchecked, NetworkChecked};
use crate::peercoin::hashes::hex::FromHex;
use crate::peercoin::secp256k1::ecdsa::Signature;
use crate::peercoin::{
...
impl<'a> RawTx for &'a Transaction {
    fn raw_hex(self) -> String {
        peercoin::consensus::encode::serialize_hex(self)
    }
}
...
pub trait RpcApi: Sized {
    ...
    fn get_block(&self, hash: &peercoin::BlockHash) -> Result<Block> {
        let hex: String = self.call("getblock", &[into_json(hash)?, 0.into()])?;
        deserialize_hex(&hex)
    }

    fn get_block_hex(&self, hash: &peercoin::BlockHash) -> Result<String> {
        self.call("getblock", &[into_json(hash)?, 0.into()])
    }

    fn get_block_info(&self, hash: &peercoin::BlockHash) -> Result<json::GetBlockResult> {
        self.call("getblock", &[into_json(hash)?, 1.into()])
    }
    //TODO(stevenroose) add getblock_txs

    fn get_block_header(&self, hash: &peercoin::BlockHash) -> Result<peercoin::block::Header> {
    ...
    fn get_best_block_hash(&self) -> Result<peercoin::BlockHash> {
        self.call("getbestblockhash", &[])
    }

    /// Get block hash at a given height
    fn get_block_hash(&self, height: u64) -> Result<peercoin::BlockHash> {
    ...
    fn get_raw_transaction(
        &self,
        txid: &peercoin::Txid,
        block_hash: Option<&peercoin::BlockHash>,
    ) -> Result<Transaction> {
    ...
    fn list_since_block(
        &self,
        blockhash: Option<&peercoin::BlockHash>,
    ...
    fn generate(&self, block_num: u64, maxtries: Option<u64>) -> Result<Vec<peercoin::BlockHash>> {
        self.call("generate", &[block_num.into(), opt_into_json(maxtries)?])
    }

    /// Mark a block as invalid by `block_hash`
    fn invalidate_block(&self, block_hash: &peercoin::BlockHash) -> Result<()> {
        self.call("invalidateblock", &[into_json(block_hash)?])
    }

    /// Mark a block as valid by `block_hash`
    fn reconsider_block(&self, block_hash: &peercoin::BlockHash) -> Result<()> {
    ...
    fn get_raw_mempool(&self) -> Result<Vec<peercoin::Txid>> {
    ...
    fn get_raw_mempool_verbose(
        &self,
    ) -> Result<HashMap<peercoin::Txid, json::GetMempoolEntryResult>> {
        self.call("getrawmempool", &[into_json(true)?])
    }

    /// Get mempool data for given transaction
    fn get_mempool_entry(&self, txid: &peercoin::Txid) -> Result<json::GetMempoolEntryResult> {
    ...
    fn send_raw_transaction<R: RawTx>(&self, tx: R) -> Result<peercoin::Txid> {
    ...
    fn wallet_process_psbt(
        ...
        let defaults = [
            true.into(),
            into_json(json::SigHashType::from(peercoin::sighash::EcdsaSighashType::All))?,
            true.into(),
        ];
    ...
    /// Submit a block
    fn submit_block(&self, block: &peercoin::Block) -> Result<()> {
        let block_hex: String = peercoin::consensus::encode::serialize_hex(block);
...
/// Client implements a JSON-RPC client for the Peercoin daemon or compatible APIs.
pub struct Client {
...
impl fmt::Debug for Client {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "peercoin_rpc::Client({:?})", self.client)
    }
}

impl Client {
    /// Creates a client to a peercoind JSON-RPC server.
    ///
...
impl RpcApi for Client {
    ...
    fn call<T: for<'a> serde::de::Deserialize<'a>>(
        ...
        if log_enabled!(Debug) {
            debug!(target: "peercoin_rpc", "JSON-RPC request: {} {}", cmd, serde_json::Value::from(args));
        }
...
fn log_response(cmd: &str, resp: &Result<jsonrpc::Response>) {
    if log_enabled!(Warn) || log_enabled!(Debug) || log_enabled!(Trace) {
        match resp {
            Err(ref e) => {
                if log_enabled!(Debug) {
                    debug!(target: "peercoin_rpc", "JSON-RPC failed parsing reply of {}: {:?}", cmd, e);
                }
            }
            Ok(ref resp) => {
                if let Some(ref e) = resp.error {
                    if log_enabled!(Debug) {
                        debug!(target: "peercoin_rpc", "JSON-RPC error for {}: {:?}", cmd, e);
                    }
                    ...
                    trace!(target: "peercoin_rpc", "JSON-RPC response for {}: {}", cmd, result);
                }
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_raw_tx() {
        use crate::peercoin::consensus::encode;
        let client = Client::new("http://localhost/".into(), Auth::None).unwrap();
        let tx: peercoin::Transaction = encode::deserialize(&Vec::<u8>::from_hex("").unwrap()).unwrap();
