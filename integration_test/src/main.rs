...
use peercoin::absolute::LockTime;
use peercoin::address::NetworkChecked;
use peercoin_rpc::json;
use peercoin_rpc::jsonrpc::error::Error as JsonRpcError;
use peercoin_rpc::{Auth, Client, Error, RpcApi};

use crate::json::BlockStatsFields as BsFields;
use peercoin::consensus::encode::{deserialize, serialize_hex};
use peercoin::hashes::hex::FromHex;
use peercoin::hashes::Hash;
use peercoin::{secp256k1, ScriptBuf, sighash};
use peercoin::{
    Address, Amount, Network, OutPoint, PrivateKey,
    Sequence, SignedAmount, Transaction, TxIn, TxOut, Txid, Witness,
};
use peercoin_rpc::peercoin_rpc_json::{
...
impl log::Log for StdLogger {
    fn enabled(&self, metadata: &log::Metadata) -> bool {
        metadata.target().contains("jsonrpc") || metadata.target().contains("peercoin_rpc")
    }
...
fn test_get_new_address(cl: &Client) {
    ...
    assert_eq!(addr.address_type(), Some(peercoin::AddressType::P2pkh));

    let addr = cl.get_new_address(None, Some(json::AddressType::Bech32)).unwrap().assume_checked();
    assert_eq!(addr.address_type(), Some(peercoin::AddressType::P2wpkh));

    let addr = cl.get_new_address(None, Some(json::AddressType::P2shSegwit)).unwrap().assume_checked();
    assert_eq!(addr.address_type(), Some(peercoin::AddressType::P2sh));
}
